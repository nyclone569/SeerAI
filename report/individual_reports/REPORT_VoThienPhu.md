...
> [!NOTE]
> **Scoring Reference** — Individual score is **40 points total**.
...
**`src/tools/get_info_tool.py` — Tool GetInfo với Gemma 4**
...
def get_stock_info(symbol: str) -> str:
    ...
    # Giới hạn output: ĐÚNG 1 câu / tối đa 50 từ — tránh phình context ReAct loop
    summary_prompt = (
...
- **Tách biệt trách nhiệm**: `GetInfo` nằm trong module độc lập `src/tools/get_info_tool.py`, không coupled với agent loop — dễ test và thay thế backend.
...
- **Tích hợp vào ReAct loop**: Agent có thể gọi tuần tự `GetInfo(VNM)` → `GetPrice(VNM)` trong 2 bước để trả lời câu hỏi phức hợp như *"Vinamilk là công ty gì và giá hôm nay bao nhiêu?"*.
...
*Phân tích sự kiện lỗi phát sinh khi tích hợp `GetInfo` — output Gemma 4 quá dài gây vòng lặp.*
...
- **Problem Description**: Khi test case `"Vinamilk là công ty gì?"` chạy trên Agent v1, Gemma 4 trong `GetInfo` trả về output >500 token. Chuỗi `current_prompt` trong ReAct loop phình to vượt context window — Gemma 4 ở bước `Thought` tiếp theo không tìm thấy `"Final Answer:"` pattern → gọi lại `GetInfo(VNM)` → lặp vô hạn đến `max_steps`.
...
- **Tách `GetInfo` thành microservice riêng**: Gemma 4 inference nặng hơn Flash — nên scale `GetInfo` service độc lập với agent orchestration layer.
...
- **Output length contract**: Mọi tool đều phải có cam kết về độ dài output tối đa (learned from `GetInfo` bug) — documented trong tool's `description` field và enforced bằng `max_tokens` param.
...
- **Redis Caching cho GetPrice + GetInfo**: TTL 60 giây cho `GetPrice`, TTL 3,600 giây cho `GetInfo` (thông tin công ty ít thay đổi) — trong môi trường nhiều user đồng thời, đây là tối ưu có ROI cao nhất.
