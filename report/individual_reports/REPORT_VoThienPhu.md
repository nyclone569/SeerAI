...
*Đóng góp cụ thể: thiết kế và triển khai tool `GetStockInfo` sử dụng Gemma 4 để tra cứu thông tin công ty niêm yết, đồng thời hoàn thiện vòng lặp ReAct trong `ReActAgent`.*
...
**`src/agent/tools.py` — Tool GetStockInfo với Gemma 4**

```python
# src/agent/tools.py
from langchain_google_genai import ChatGoogleGenerativeAI

# Instance Gemma 4 riêng cho GetStockInfo — tách biệt khỏi LLM ReAct loop chính
...
**Đăng ký `GetStockInfo` vào tool registry của agent**
...
tools = [
    ...
    Tool(name="GetStockInfo",     func=get_stock_info,     description="Lấy thông tin tổng quan công ty: tên, ngành, vốn hoá, sàn niêm yết. Dùng Gemma 4 để tóm tắt."),
]
```

### I.3 Code Quality Evidence

- **Tách biệt trách nhiệm**: `GetStockInfo` nằm trong module độc lập `src/agent/tools.py`, không coupled với agent loop — dễ test và thay thế backend.
...
- **Tích hợp vào ReAct loop**: Agent có thể gọi tuần tự `GetStockInfo(VNM)` → `GetPrice(VNM)` trong 2 bước để trả lời câu hỏi phức hợp như *"Vinamilk là công ty gì và giá hôm nay bao nhiêu?"*.
...
*Phân tích sự kiện lỗi phát sinh khi tích hợp `GetStockInfo` — output Gemma 4 quá dài gây vòng lặp.*
...
- **Problem Description**: Khi test case `"Vinamilk là công ty gì?"` chạy trên Agent v1, Gemma 4 trong `GetStockInfo` trả về output >500 token. Chuỗi `current_prompt` trong ReAct loop phình to vượt context window — Gemma 4 ở bước `Thought` tiếp theo không tìm thấy `"Final Answer:"` pattern → gọi lại `GetStockInfo(VNM)` → lặp vô hạn đến `max_steps`.
...
{"event": "TOOL_CALL",    "tool": "GetStockInfo", "args": "VNM", "result": "📊 VNM — [542 tokens...]", "step": 1},
{"event": "TOOL_CALL",    "tool": "GetStockInfo", "args": "VNM", "result": "📊 VNM — [538 tokens...]", "step": 2},
{"event": "TOOL_CALL",    "tool": "GetStockInfo", "args": "VNM", "result": "📊 VNM — [531 tokens...]", "step": 3},
...
| Layer | Diagnosis |
| :--- | :--- |
| **Prompt (GetStockInfo)** | `summary_prompt` không giới hạn độ dài → Gemma 4 sinh 500+ token mỗi lần |
...
```json
{"event": "TOOL_CALL", "tool": "GetStockInfo", "args": "VNM", "result": "📊 VNM — Vinamilk là công ty sữa hàng đầu Việt Nam, niêm yết HOSE, vốn hoá ~152,000 tỷ VND.", "tokens": 38},
...
- **Khi Observation bị mất (truncated)**: Agent bị stuck — đây chính là bug tôi phát hiện và fix với `GetStockInfo`.
...
- **Tách `GetStockInfo` thành microservice riêng**: Gemma 4 inference nặng hơn Flash — nên scale `GetStockInfo` service độc lập với agent orchestration layer.
...
- **Output length contract**: Mọi tool đều phải có cam kết về độ dài output tối đa (learned from `GetStockInfo` bug) — documented trong tool's `description` field và enforced bằng `max_tokens` param.
...
- **Redis Caching cho GetPrice + GetStockInfo**: TTL 60 giây cho `GetPrice`, TTL 3,600 giây cho `GetStockInfo` (thông tin công ty ít thay đổi) — trong môi trường nhiều user đồng thời, đây là tối ưu có ROI cao nhất.
